...
class TrajectoryUtils():
    ...
    def sampleTrajectoryThroughWaypoints(self, trajectory:Trajectory, with_stops, smooth_path, smoothing_la_dist, smoothing_sampling_step, velocity_limits, acceleration_limits):
        ...
        if with_stops:
            ...
            tol = 0.1 #add tol to limits to not violate constraints by rounding errors
            ...
            samples = [toppra_trajectory.eval(t) for t in np.linspace(0, duration, int(duration/sampling_step))] # [STUDENTS TODO] Fill this variable with trajectory samples
